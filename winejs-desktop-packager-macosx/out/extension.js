...
function activate(context) {
    ...
    const disposable = vscode.commands.registerCommand('winejs.createDesktopApp', async (uri) => {
        if (!uri || !uri.fsPath) {
...
async function createDesktopApp(folderPath, context) {
    ...
    async function showIconSelectionGrid(icons) {
        return new Promise(async (resolve) => {
            const panel = vscode.window.createWebviewPanel('iconSelection', 'Select App Icon', vscode.ViewColumn.Active, { enableScripts: true });
    ...
    async function showSingleIconPreview(iconPath) {
        return new Promise(async (resolve) => {
            const panel = vscode.window.createWebviewPanel('iconPreview', 'Icon Preview', vscode.ViewColumn.Active, { enableScripts: true });
    ...
    if (!iconExtracted || !fs.existsSync(iconPath)) {
        await vscode.window.withProgress({
            ...
            try {
                const https = require('https');
                const iconBuffer = await new Promise((resolve, reject) => {
                    https.get('https://cdn.gitgpt.chat/rtx/images/wine-placeholder.png', (res) => {
                        const chunks = [];
                        res.on('data', (chunk) => chunks.push(chunk));
                        res.on('end', () => resolve(Buffer.concat(chunks)));
                        res.on('error', reject);
    ...
    const category = await vscode.window.showQuickPick(['Game', 'Graphics', 'Audio', 'Utility', 'Office', 'Development', 'Other'], { placeHolder: 'Select app category (for Finder classification)', title: 'App Category' });
    ...
    const useSymlink = config.get('useSymlink', true);
    let appBundlePath = ""; // Initialize with empty string
    await vscode.window.withProgress({
        location: vscode.ProgressLocation.Notification,
...
- **No terminal window appears** - runs silently in background!
    ...
    vscode.window.showInformationMessage(`✅ "${appName}" desktop app created! (No terminal mode)`, "Open Desktop", "Open Applications").then(selection => {
...
function generateLauncherScript(appName, mainExeName, mainExeRelPath, hasRegFile, hasSetupBat, hasPrelaunchSh, useSymlink) {
...
function generateInfoPlist(appName, version, category, bundleId) {
    const categoryMap = {
...
async function convertPngToIcns(pngPath, icnsPath) {
...
async function createInstallScripts(appName, appBundleName, applicationsPath, desktopPath) {
    const escapedAppName = appName.replace(/ /g, '_');
...
`;
    ...
    vscode.window.showInformationMessage(`📦 Install/Uninstall scripts created in ~/.winejs-scripts/`, "Open Scripts Folder").then(selection => {
        if (selection === "Open Scripts Folder") {
            vscode.env.openExternal(vscode.Uri.file(scriptsDir));
        }
    });
}
async function addToDock(appName, appPath) {
    try {
