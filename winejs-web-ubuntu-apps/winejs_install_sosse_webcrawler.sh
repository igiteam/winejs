...
SOSSE_LOGO_URL="https://cdn.gitgpt.chat/rtx/images/sosse-logo.png"
...
docker exec winejs-sosse sosse-admin migrate 2>/dev/null || true
docker exec winejs-sosse sosse-admin update_se 2>/dev/null || true
docker exec winejs-sosse sosse-admin update_mime 2>/dev/null || true

# Create admin user
log "👤 Creating admin user..."
docker exec winejs-sosse sosse-admin default_admin 2>/dev/null || true

# Set admin password using Django shell
docker exec winejs-sosse python3 manage.py shell << PYTHON_EOF
from django.contrib.auth.models import User
try:
    user = User.objects.get(username='${ADMIN_USER}')
    user.set_password('${ADMIN_PASSWORD}')
    user.save()
    print(f"Password updated for user: ${ADMIN_USER}")
except User.DoesNotExist:
    User.objects.create_superuser('${ADMIN_USER}', 'admin@example.com', '${ADMIN_PASSWORD}')
    print(f"Superuser created: ${ADMIN_USER}")
PYTHON_EOF

log "✅ Sosse initialized"
...
if curl -L "$SOSSE_LOGO_URL" -o "$ICON_DIR/sosse.png" 2>/dev/null; then
    success "✅ Icon downloaded successfully"
else
    warn "Failed to download icon, creating placeholder..."
    cat > "$ICON_DIR/sosse.svg" << 'SVG_EOF'
